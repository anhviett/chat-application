...
import { GeminiMessage } from "@/types/gemini-message";
...
export function useGeminiAi() {
   ...
   */
  const sendMessage = async (prompt: string, history: GeminiMessage[] = []) => {
    ...
    try {
      const response = await fetch(
        import.meta.env.VITE_BACKEND_URL + "/chat-gemini/stream",
        {
          method: "POST",
