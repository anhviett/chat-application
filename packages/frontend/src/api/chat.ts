...
import { ConversationType } from "@/types/conversation-type.ts";
...
export const chatApi = {
  ...
  sendMessage: async function (data: SendMessage, cancel = false) {
    const response = await api.request({
      ...
      data: data,
      signal: cancel
        ? cancelApiObject[this.sendMessage.name].handleRequestCancellation()
            .signal
        : undefined,
    });
    return response.data;
  ...
  getConversationMessages: async function (
    ...
    const response = await api.request({
      ...
      signal: cancel
        ? cancelApiObject[
            this.getConversationMessages.name
          ].handleRequestCancellation().signal
        : undefined,
    });
